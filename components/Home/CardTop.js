...
const CardTop = ({ title, active, index, audio, setCurrentStep, type }) => {
  ...
  const sendListeningDataToServer = async (userId, trackId, seconds) => {
    try {
      const { data, error } = await supabase.from("listening_stats").insert([
        {
          user_id: userId,
          audio_id: trackId,
          second: seconds,
  ...
  return (
    <>
      <View
        style={[
          styles.cardConteiner,
          {
            backgroundColor:
              active === index ? "#FFFFFF" : "rgba(255, 255, 255, 0.15)",
          },
        ]}
      >
        <Text
